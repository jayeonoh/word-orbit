...
function renderHome() {
  ...
  return `
    ...
    <section class="panel mission">
      ...
      ${total ? `<button class="primary big" id="startSession">오늘의 학습 시작 →</button>` : `<p class="info-line">오늘 복습할 단어는 다 했어요. ${state.words.length ? '더 하고 싶으면 아래 연습을 눌러요.' : '단어를 추가해 볼까요?'}</p>`}
      ${practiceItems().length ? `<button class="${total ? 'ghost' : 'primary'} big" id="startPractice">✨ 더 연습하기 (${practiceItems().length}개)</button>` : ''}
    </section>
...
// 홈에서 바로 하는 연습 회차: 아직 안 만난 새 단어 조금 + 기존 단어(복습일 가까운 순). 일정은 바꾸지 않음
function practiceItems() {
  const fresh = state.words.filter(w => w.progress.attempts === 0).slice(0, Math.max(1, Math.ceil((state.profile.newPerDay || 4) / 2))).map(w => ({ id: w.id, intro: true }));
  const existing = state.words.filter(w => w.progress.attempts > 0 && !w.progress.mastered).sort((a, b) => a.progress.due.localeCompare(b.progress.due)).slice(0, 8).map(w => ({ id: w.id, intro: false, practice: true }));
  const mastered = state.words.filter(w => w.progress.mastered).slice(0, 2).map(w => ({ id: w.id, intro: false, practice: true }));
  return shuffle([...fresh, ...existing, ...mastered]);
}
function startPractice() {
  const queue = practiceItems(); if (!queue.length) return;
  state.session = { queue, index: 0, results: [], phase: 'intro', answer: '', hinted: false, feedback: null, options: null, simple: '', rec: null, round: 2 };
  prepareItem(); render(); window.scrollTo(0, 0);
}
function bindHome() {
  const b = $('#startSession'); if (b) b.onclick = startSession;
  const pr = $('#startPractice'); if (pr) pr.onclick = startPractice;
...
function renderAdd() {
  ...
  else if (!add.manual.advanced && key) body = `
    <p class="muted">영어 단어만 적으면 아이 수준에 맞는 뜻·예문·한국어를 AI가 만들어요. 여러 개는 쉼표로 나눠 적어도 돼요.</p>
    <label class="wide">단어 (영어)<input id="m-word" value="${esc(add.manual.word || '')}" placeholder="enormous, curious"></label>
    <label class="wide">책에서 본 문장 <span class="muted small">(선택 — 적으면 그 문장에 맞는 뜻으로 만들어요)</span><input id="m-ctx" value="${esc(add.manual.context || '')}" placeholder="An enormous elephant walked to the river."></label>
    ${state.busy ? `<p class="progress-line" id="aiProgress">${esc(state.progress || '만드는 중…')}</p>` : ''}
    <button class="primary big" id="lookupWord" ${state.busy ? 'disabled' : ''}>${state.busy ? '만드는 중…' : '✨ 뜻 만들기'}</button>
    <p class="muted small center"><button class="text-link" id="toAdvanced">AI 없이 내가 다 입력할래요</button></p>`;
  else body = `
    <div class="form-grid">
      ...
      <label>주제<select id="m-topic">${TOPICS.map(t => `<option value="${t.id}" ${add.manual.topic === t.id ? 'selected' : ''}>${t.name}</option>`).join('')}</select></label>
    </div>
    <p class="muted small">오답 보기는 다른 단어의 뜻에서 자동으로 만들어요.</p>
    <div class="button-row"><button class="primary" id="addManual">단어 추가</button>${key ? '<button class="ghost" id="toSimple">✨ AI로 만들기</button>' : ''}</div>`;
...
function bindAdd() {
  ...
  const ta = $('#toAdvanced'); if (ta) ta.onclick = () => { add.manual = { ...add.manual, word: $('#m-word')?.value || '', context: $('#m-ctx')?.value || '', advanced: true }; render(); };
  const ts = $('#toSimple'); if (ts) ts.onclick = () => { add.manual = { ...add.manual, word: $('#m-word')?.value || '', context: $('#m-ctx')?.value || '', advanced: false }; render(); };
  const lw = $('#lookupWord'); if (lw) lw.onclick = async () => {
    const raw = $('#m-word').value.trim(); const ctx = $('#m-ctx').value.trim();
    if (!raw) return toast('단어를 먼저 적어 주세요.');
    const list = raw.split(/[,;\n]+/).map(x => x.trim()).filter(Boolean).slice(0, 8);
    add.manual = { ...add.manual, word: raw, context: ctx };
    state.busy = true; state.progress = '뜻을 만드는 중…'; render();
    try {
      const r = await ai.lookupWords({ key: ai.getKey(), words: list, context: ctx, age: state.profile.age, level: state.profile.level });
      const got = (r.words || []).filter(x => x.word && x.definition).map(x => ({ ...x, checked: true, distractors: Array.isArray(x.distractors) ? x.distractors : [] }));
      const have = new Set(add.found.map(f => f.word.toLowerCase()));
      add.found.push(...got.filter(g => !have.has(g.word.toLowerCase())));
      add.note = r.note || '';
      if (got.length) { add.manual = { advanced: false }; toast(`${got.length}개 단어를 만들었어요. 확인하고 단어장에 넣어요.`, 'success'); }
      else toast('뜻을 만들지 못했어요. 철자를 확인하고 다시 시도해 주세요.');
    } catch (err) { toast(err.message, 'error'); }
    state.busy = false; state.progress = ''; render();
    setTimeout(() => document.getElementById('addFound')?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 100);
  };
  const am = $('#addManual'); if (am) am.onclick = async () => {
    ...
    const n = await addWords([w]); toast(n ? `“${w.word}”를 추가했어요.` : '이미 있는 단어예요.', n ? 'success' : 'info'); add.manual = { advanced: true }; render();
  };
